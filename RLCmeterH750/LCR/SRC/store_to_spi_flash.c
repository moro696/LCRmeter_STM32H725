...
//The size of the field that stores the size of the structure
#define STRUCT_SS 2
...
bool SpiFlashWriteToFlash(uint16_t flash_sector, uint16_t struct_size, void* void_data)
{
    ...
    if(struct_size >0xFF)
    {
    	m25p16_read(flash_offset+0, STRUCT_SS, &struct_size_flash); // read 16bit structure size from flash
    }
    else
    {

    m25p16_read(flash_offset+0, STRUCT_SS-1, &struct_size_flash); // read 8bit structure size from flash
    }
#else
    m25p16_read(flash_offset+0, STRUCT_SS, &struct_size_flash); // read structure size from flash
#endif
  uint16_t bb=0;

    //if the actual structure size from flash is diferent from the initial structure size, then we erase the
    // sector and store it in flash at address 0x000000;
    if(struct_size_flash!=struct_size)
    {

    	m25p16_ram_erase64k(flash_offset);

    	flash_unlock_allSectors();//for debug only

#ifdef ST25V016

    	 if(struct_size >0xFF)
    	 {
    		 uint16_t dd= struct_size;
    		 uint16_t ms= dd>>8;
    		 uint16_t ls= dd&0xFF;

    		 m25p16_read(flash_offset+0, STRUCT_SS, &bb); //for debug only
    		 m25p16_write(flash_offset+0, 1, &ls); // save structure size in 8bit to flash
    		 m25p16_write(flash_offset+1, 1, &ms); // save structure size in 8bit to flash
    	 }
    	 else
    	 {
    		 m25p16_write(flash_offset+0, STRUCT_SS-1, &struct_size); // save structure size in 8bit to flash
    	 }
    	...
    	m25p16_write(flash_offset+0, STRUCT_SS, &struct_size); // write new struct size( 0x18)  at addr 0x000000
    	m25p16_read(flash_offset+0, STRUCT_SS, &bb); //for debug only
#endif
   ...
   // m25p16_write(flash_offset+parts_offset, struct_size, void_data);
    ...
    uint32_t tAddr=flash_offset+parts_offset;

    for(int z=0;z<struct_size;z++)
    {
       	m25p16_write(tAddr, 1, void_data);
       	void_data++;
       	tAddr++;
    }

   // uint32_t fifi[1024];
   // m25p16_read(flash_offset+parts_offset, struct_size, &fifi);//debug only
...
#ifdef ST25V016
    m25p16_read(flash_offset+0, STRUCT_SS-1, &struct_size_flash);
#else
...
bool SpiFlashReadFromFlash2(uint16_t flash_sector, uint16_t struct_size, void *void_data)
{
    if(struct_size==0)
        return false;
