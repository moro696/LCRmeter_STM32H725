...
#define RAM_ERASE_FULL 0x60 // moro - full chip erase
#define RAM_ERASE64K 0xD8	// erase 64 kilobyte data
#define RAM_ERASE4K 0x20	// erase 4 kilobyte data
...
#define RAM_WREN_STATUS 0x50 // moro - write enable for status register
#define RAM_AAI_EN 0xAD // moro - enable address autoincrement
...
#define ID0_MICROCHIP 0xBF   //moro
...
bool m25p16_read_ram_id_and_check()
{
    ...
    if(!(id[0]==ID0_WINBOND || id[0]==ID0_MICRON || id[0]== ID0_MICROCHIP))
        return false;
   ...
   /* return (id[1]==0x20 || id[1]==0x40) &&
              (id[2]>=16 && id[2]<=37);
   */
    return (id[1]==0x20 || id[1]==0x40 || id[1]==0x25) &&
           (id[2]>=16 && id[2]<=65);
}
...
void m25p16_write_flash_ram_autoincrementAddress(M25P16_RW_TYPE rw_type, uint16_t bytes_to_readwrite, uint8_t flash_sector, uint8_t flash_page, uint8_t offset,
                                 uint8_t* mem_ptr)
{

	       // send WREN instruction
	        CS_ZERO
	        xmit_spi(RAM_WREN);				// 0x06 - write enable instruction
	        CS_ONE
	        DelayUs(2);

	        CS_ZERO
			xmit_spi( RAM_AAI_EN);				// 0xAD - enable address autoincrement
	        CS_ONE
			DelayUs(2);
...
void flash_unlock_allSectors(void)
{


	//readStatusRegister();
	 CS_ZERO;
		 xmit_spi(RAM_WREN); // 0x06 to enable writing
	    CS_ONE;
	    DelayUs(1);
	 CS_ZERO;
	 xmit_spi(RAM_WREN_STATUS); // send 0x50 - write status register enable
	 CS_ONE;
	 DelayUs(1);

	 CS_ZERO;
	 xmit_spi(0x01); // send 0x01 - write to  status register
	// CS_ONE;
	// DelayUs(1);

	// CS_ZERO;
	 xmit_spi(0x00); // write status register to 0 - this will unlock all flash sectors
	 CS_ONE;
	 DelayUs(1);
...
void flash_lock_allSectors(void)
{
	 CS_ZERO;
	 xmit_spi(RAM_WREN_STATUS); // send 0x50 - write status register enable
	 CS_ONE;
	 ...
	 CS_ZERO;
	 xmit_spi(RAM_WRSR); // send 0x01 - write to  status register
	 xmit_spi(0x3C); // write status register to 0 - this will lock all flash sectors
	 CS_ONE;
	 DelayUs(1);
...
void moro_test(void){
	uint32_t address=0;
	 uint8_t kk[3]={0x69,0x55,0x66};
//	 uint8_t tmp=0;
	 uint16_t mm;
	 m25p16_read(0, 2, &mm);
	 flash_unlock_allSectors();
	// readStatusRegister();
	 m25p16_ram_erase64k(0);

	 m25p16_write(10, 1, &kk); // debug only
	 m25p16_write(11, 1, &kk[1]);
...
void m25p16_write_buffer(uint32_t offset, uint16_t size, const void* mem_ptr)
{
	...
	CS_ZERO
	xmit_spi(RAM_AAI_EN); // 0xAD write enable word instruction
	CS_ONE
	DelayUs(2);
