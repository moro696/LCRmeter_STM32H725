...
//	return g_corrections.magic && g_corrections.size;
...
{
    g_corrections.magic = MAGIC_DATA;
    g_corrections.size = sizeof(CorrectionsAll); //always returns 5824
...
void CorrectionLoad(uint32_t index)
{
    ...
    SpiFlashReadFromFlash2(FLASH_SECTOR_CORRECTION+index, sizeof(CorrectionsAll), &g_corrections);
